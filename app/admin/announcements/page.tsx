  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotificationStatus({
        show: true,
        message: "Announcement created! Sending email notifications to group members...",
        type: 'info'
      })
      
      // Check notification status after a delay
      setTimeout(async () => {
        try {
        ...
        <div className="h-[calc(100vh-8rem)] flex flex-col bg-gray-50">
          ...
          {notificationStatus?.show && (
            <div className={`
              ...
              ${notificationStatus.type === 'info' ? 'bg-blue-100 text-blue-800 border border-blue-200' : ''}
            ...
            `}>
              <div className="flex items-start space-x-3">
                <div className="flex-shrink-0">
                  {notificationStatus.type === 'success' && <CheckCircle className="h-5 w-5 text-green-600" />}
                  {notificationStatus.type === 'info' && (
                    <div className="flex items-center">
                      <Mail className="h-5 w-5 text-blue-600 animate-pulse" />
                    </div>
                  )}
                  {notificationStatus.type === 'warning' && <AlertCircle className="h-5 w-5 text-yellow-600" />}
                  {notificationStatus.type === 'error' && <XCircle className="h-5 w-5 text-red-600" />}
                </div>
                <div className="flex-1">
                  <p className="font-medium">{notificationStatus.message}</p>
                  {notificationStatus.details && (
                    <p className="text-sm mt-1 opacity-90">
                      {notificationStatus.details.notified && (
                        <>
                          Sent to {notificationStatus.details.notified} student{notificationStatus.details.notified !== 1 ? 's' : ''}
                          {notificationStatus.details.failed && notificationStatus.details.failed > 0 && (
                            <>, {notificationStatus.details.failed} failed</>
                          )}
                          {notificationStatus.details.total && (
                            <> out of {notificationStatus.details.total} total</>
                          )}
                        </>
                      )}
                    </p>
              ...
              >
                ...
                <DialogContent className="max-w-lg">
                  <DialogHeader>
                    <DialogTitle>Send New Announcement</DialogTitle>
                    <DialogDescription>
                      Share a message with your group. Students will receive email notifications.
                    </DialogDescription>
                  </DialogHeader>
                  <form onSubmit={handleSubmit} className="space-y-4">
                    {error && (
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      <div className="flex items-start space-x-2">
                        ...
                        <p className="text-sm text-blue-800">
                          Students in this group will receive email notifications about this announcement.
                        </p>
                      </div>
                    </div>
                            ...
                            <div className="space-y-2">
                              {announcement.files.map((file) => (
                                <a
                                  ...
                                  className="bg-green-600 rounded-lg p-3 flex items-center justify-between hover:bg-green-700 transition-colors"
